...
/**
 * Sensor lock state
 * // TODO: RAW Sensor Lock is a Boon (3D keep 2), not a flat DM.
 * // See lib/operations/combat-engine.js:40-49 for correct implementation.
 * Locked targets get +1 to hit with guided weapons
 */
const SENSOR_LOCK_BONUS = 1;

/**
 * Calculate sensor lock attack bonus
...
function calculateSensorLockBonus(attackerState, targetId, isGuidedWeapon = false) {
  if (!isGuidedWeapon) return 0;
  if (attackerState?.sensorLock === targetId) return SENSOR_LOCK_BONUS;
  return 0;
}
