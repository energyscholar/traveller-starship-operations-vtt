 ...
 */
function applyCriticalHit(ship, location, severity, failureReason = null) {
  ...
  // Build crit entry with optional failure reason
  const critEntry = {
    severity,
    repaired: false,
    timestamp: Date.now()
  ...
  // AR-194: Store failure reason if provided
  if (failureReason) {
    critEntry.failureReason = {
      id: failureReason.id,
      name: failureReason.name,
      description: failureReason.description,
      flavorText: failureReason.flavorText
    };
  }

  // Add the crit to the location's array
  ship.crits[location].push(critEntry);
