...
const MISSILE_TYPES = {
  ...
  // AR-EPSILON-B: Bomb-pumped torpedo (TL15)
  bomb_pumped_torpedo: {
    damage: '6D6',           // Higher damage than standard missiles
    radiation: true,
    radiationDamage: '3D6',  // Enhanced radiation damage
    cost: 750000,
    legal: false,
    smart: true,
    tl: 15,
    bypassDamper: true,      // Detonates beyond nuclear damper effective range
    description: 'Bomb-pumped laser torpedo - detonates outside damper range, bypasses nuclear dampers'
  }
};
 ...
 * @param {boolean} bypassDamper - AR-EPSILON-B: Weapon bypasses nuclear dampers (bomb-pumped torpedo)
 * @returns {Object} { applies: boolean, dm: number, reason: string }
 */
function checkRadiationDamage(targetArmor, hasNuclearDamper = false, hasRadiationShielding = false, bypassDamper = false) {
  // AR-EPSILON-B: Bomb-pumped torpedoes bypass nuclear dampers
  if (hasNuclearDamper && !bypassDamper) {
    return {
      applies: false,
      dm: 0,
      reason: 'Nuclear damper negated radiation'
    };
  }

  // Radiation shielding still works against bomb-pumped torpedoes
  if (hasRadiationShielding) {
    return {
      applies: false,
      dm: 0,
      reason: 'Radiation shielding protected crew'
    };
  }
  ...
  return {
    ...
    reason: bypassDamper && hasNuclearDamper
      ? `Bomb-pumped torpedo bypassed nuclear damper! Radiation crew hit with -${targetArmor} DM from armor`
      : `Radiation crew hit with -${targetArmor} DM from armor`
  };
}
