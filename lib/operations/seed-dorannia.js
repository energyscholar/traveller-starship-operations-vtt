...
function seedDoranniaCampaign() {
  ...
  if (existingCampaign) {
    ...
    db.prepare(`
      UPDATE campaigns SET current_date = ?, current_system = ?, updated_at = CURRENT_TIMESTAMP
      WHERE id = ?
    `).run(CAMPAIGN.current_date, CAMPAIGN.current_system, campaignId);
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?)
    `).run(campaignId, CAMPAIGN.name, CAMPAIGN.gm_name, CAMPAIGN.current_date, CAMPAIGN.current_system);
  ...
  const insertContact = db.prepare(`
    INSERT INTO contacts (id, campaign_id, name, type, bearing, range_km, range_band, transponder, signature, visible_to, gm_notes, is_targetable, weapons_free, health, max_health)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
