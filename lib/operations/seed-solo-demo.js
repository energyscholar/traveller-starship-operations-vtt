 ...
 * Removes only campaigns with "Solo Demo" in the name that don't have the canonical ID.
 * This prevents duplicates appearing in campaign list.
 * NOTE: Only matches by name to avoid accidentally deleting campaigns that just
 * have campaign_type='solo_demo' but are real campaigns (like Tuesday Spinward).
 */
function cleanupLegacySoloDemos() {
  // Find legacy solo demo campaigns (by name ONLY, excluding canonical ID)
  // We only match by name because that's what causes the display duplicate
  const legacyCampaigns = db.prepare(`
    SELECT id, name FROM campaigns
    WHERE name LIKE '%Solo Demo%'
    AND id != ?
