...
let db, cleanupTestData, generateId;
let seedDoranniaCampaign, CAMPAIGN;

try {
  console.log('[RESET-DB] Loading database module...');
  ({ db, cleanupTestData, generateId } = require('./database'));
...
try {
  ...
  ({ seedDoranniaCampaign, CAMPAIGN } = require('./seed-dorannia'));
