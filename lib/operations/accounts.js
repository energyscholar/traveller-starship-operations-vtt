...
const CREW_ROLES = {
  ...
  // AR-291: Observer role - view-only access, can switch to other roles
  SPECIAL: [
    'observer'
  ],
  CUSTOM: 'custom'
...
const ALL_ROLES = [...CREW_ROLES.COMBAT, ...CREW_ROLES.SUPPORT, ...CREW_ROLES.SPECIAL];
...
function updateCampaign(id, updates) {
  const allowedFields = [
    ...
    'campaign_type'  // AR-241: Solo explorer campaign type
  ];
  const setClause = [];
  const values = [];
