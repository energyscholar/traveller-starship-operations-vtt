...
function rollDamage(diceStr) {
  ...
  // Ion cannon damage: 'ion' = 2d6, 'ion+1d' = 3d6 (High Yield)
  // Power drain = total × 10 (Effect added by caller after attack roll)
  if (diceStr.toLowerCase().startsWith('ion')) {
    const ionDice = diceStr.includes('+1d') ? 3 : 2;
    const dice = [];
    for (let i = 0; i < ionDice; i++) {
      dice.push(roll1D6());
    }
    ...
    return {
      ...
      modifier: 0,
      total,
      powerDrain: total * 10  // Base drain; Effect multiplier applied after hit
    };
  }
...
function resolveAttack(attacker, weapon, target, modifiers = {}) {
  ...
  // Ion weapons deal power drain, not hull damage
  const isIonWeapon = damageRoll.type === 'ion';
  const actualDamage = isIonWeapon ? 0 : Math.max(0, damageRoll.total - armor);
  ...
  // Ion power drain: (dice total + Effect) × 10
  if (isIonWeapon) {
    result.powerDrain = (damageRoll.total + Math.max(0, effect)) * 10;
  }

  // Check for critical hit (effect 6+) - not for ion weapons
  result.critical = !isIonWeapon && effect >= 6;
  if (result.critical) {
