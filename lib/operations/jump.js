...
function initiateJump(shipId, campaignId, destination, distance, destinationHex = null, destinationSector = null) {
  ...
  for (const type of consumeOrder) {
    ...
    // AR-EPSILON-B: Track if unrefined fuel was consumed
    if (type === 'unrefined' && consume > 0) {
      usedUnrefined = true;
    }
  }
  ...
  updateShipState(shipId, {
    jump: {
      ...
      usedUnrefined  // AR-EPSILON-B: Track fuel quality for emergence check
    },
    fuel: newFuel,
    fuelBreakdown
  });
...
function completeJump(shipId, campaignId) {
  ...
  // AR-EPSILON-B: Post-jump engineer check with fuel quality modifier
  const emergenceCheck = performEmergenceCheck(ship, jump);
  ...
  return {
    ...
    // AR-EPSILON-B: Emergence check results
    emergenceCheck
  };
}

/**
 * AR-EPSILON-B: Perform post-jump emergence check
 * Engineer check affected by fuel quality used during jump
 * @param {Object} ship - Ship object
 * @param {Object} jump - Jump state
 * @returns {Object} Emergence check result
 */
function performEmergenceCheck(ship, jump) {
  const { skillCheck } = require('../skill-checks');

  // Get fuel quality modifier
  const fuelBreakdown = ship.current_state?.fuelBreakdown || {};
  const usedUnrefined = jump.usedUnrefined || false;
  const fuelModifier = usedUnrefined ? -2 : 0;

  // Engineer check (Average 8+) with fuel modifier
  const result = skillCheck({
    skillLevel: 1, // Default engineer skill (would use actual crew skill if available)
    characteristic: 0,
    difficulty: 8,
    modifiers: fuelModifier !== 0 ? [{ reason: 'Unrefined fuel', dm: fuelModifier }] : []
  });
  ...
  return {
    ...
    message: result.success
      ? 'Jump emergence nominal. All systems green.'
      : `Jump emergence rough. ${result.criticalFailure ? 'CRITICAL: Drive strain detected!' : 'Minor drive stress.'}`
  };
}
