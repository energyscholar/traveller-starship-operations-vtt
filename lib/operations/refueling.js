...
function getFuelStatus(shipId) {
  ...
  return {
    ...
    fuelProcessor: shipData.fuelProcessor || false
  };
}
...
function getAvailableSources(campaignId) {
  ...
  // Assume basic starport available (would check UWP in real implementation)
  sources.push({
...
function canProcessFuel(shipId, tons) {
  ...
  // Check if ship has fuel processor
  const hasFuelProcessor = ship.ship_data?.fuelProcessor ||
                          ship.ship_data?.fuelScoops ||  // Scoops often include processor
                          false;
