...
// ============ AR-38: NPC Email Templates ============

// NPC Types for automated emails
const NPC_TYPES = {
  PATRON: 'patron',
  AUTHORITY: 'authority',
  SERVICES: 'services',
...
const NPC_TEMPLATES = {
  ...
  patron: {
    ...
    payment: (data) => ({
      subject: `Payment Received - ${data.jobReference || 'JOB'}`,
      body: `${data.patronName} has transferred Cr${(data.amount || 0).toLocaleString()} to your account.\n\nReference: ${data.jobReference || 'N/A'}`
    })
  },
  // Authority emails
  authority: {
    berthing: (data) => ({
      subject: `Berthing Assignment - ${data.berthNumber || 'TBD'}`,
      body: `Welcome to ${data.starportName}.\n\nYou have been assigned berth ${data.berthNumber} for ${data.duration || 'TBD'}.`
    }),
    customs_inspection: (data) => ({
      subject: 'Customs Inspection Notice',
      body: `Your vessel is scheduled for inspection at ${data.inspectionTime || 'TBD'}.\n\nRequired documents: ${(data.requiredDocs || []).join(', ')}`
    }),
    fine: (data) => ({
      subject: 'Official Notice - Fine Assessment',
      body: `Offense: ${data.offense || 'Violation'}\n\nFine amount: Cr${(data.amount || 0).toLocaleString()}\nDue: ${data.dueDate || '7 days'}`,
      priority: 'high'
    }),
    departure_clearance: (data) => ({
  ...
  // Ship services emails
  services: {
    fuel_quote: (data) => ({
      subject: 'Fuel Quote',
      body: `${data.companyName || 'Fuel Services'} quote:\n\n${data.fuelType || 'Refined'} fuel: Cr${(data.pricePerTon || 500).toLocaleString()}/ton\nQuantity: ${data.quantity || 0} tons\nTotal: Cr${((data.pricePerTon || 500) * (data.quantity || 0)).toLocaleString()}`
    }),
    repair_estimate: (data) => ({
      subject: 'Repair Estimate',
      body: `${data.yardName || 'Repair Yard'} estimate:\n\nRepairs:\n${(data.repairs || []).map(r => `- ${r}`).join('\n')}\n\nTotal: Cr${(data.totalCost || 0).toLocaleString()}\nTime: ${data.timeEstimate || 'TBD'}`
    }),
    cargo_listing: (data) => ({
      subject: `Cargo Available: ${data.cargo || 'Goods'}`,
      body: `${data.brokerName || 'Broker'} listing:\n\nCargo: ${data.cargo}\nTons: ${data.tons || 0}\nDestination: ${data.destination || 'TBD'}\nPayment: Cr${(data.payment || 0).toLocaleString()}`
    })
  },
  // Traffic control emails
  traffic_control: {
    departure_vector: (data) => ({
      subject: `Traffic Advisory: ${data.clearanceCode || 'CLEAR'}`,
      body: `${data.starportName || 'Control'} Traffic Control:\n\nVector: ${data.vector || 'TBD'}\nClearance: ${data.clearanceCode}`
    })
  }
};

/**
 * AR-38: Generate NPC email from template
 * @param {string} npcType - Type of NPC (patron, authority, services, traffic_control)
 * @param {string} templateType - Template name (e.g., 'job_offer', 'berthing')
 * @param {Object} data - Template data
 * @returns {Object} Generated email content { subject, body, priority? }
 */
function generateNPCEmail(npcType, templateType, data) {
  const typeTemplates = NPC_TEMPLATES[npcType];
  if (!typeTemplates) {
    throw new Error(`Unknown NPC type: ${npcType}`);
  }
  const template = typeTemplates[templateType];
  if (!template) {
    throw new Error(`Unknown template ${templateType} for NPC type ${npcType}`);
  }
  return template(data);
}

// ============ AR-38: Mailstorm Prevention ============

// AI exchange limit per thread (2 back-and-forth = 4 messages)
const AI_EXCHANGE_LIMIT = 4;

// Track AI exchanges per thread (in-memory, resets on restart)
const threadAIExchanges = new Map();

/**
 * AR-38: Check if AI can send message in thread (mailstorm prevention)
 * @param {string} threadId - Thread ID
 * @param {boolean} isPlayerMessage - True if message is from player
 * @returns {Object} { allowed: boolean, reason?: string }
 */
function checkAIExchangeLimit(threadId, isPlayerMessage = false) {
  if (isPlayerMessage) {
    // Player message resets the counter
    threadAIExchanges.set(threadId, 0);
    return { allowed: true };
  }

  const currentCount = threadAIExchanges.get(threadId) || 0;
  if (currentCount >= AI_EXCHANGE_LIMIT) {
    return { allowed: false, reason: 'ai_exchange_limit', message: 'AI exchange limit reached (2 back-and-forth max)' };
  }

  threadAIExchanges.set(threadId, currentCount + 1);
  return { allowed: true };
}

/**
 * AR-38: Get AI exchange count for a thread
 * @param {string} threadId - Thread ID
 * @returns {number} Current exchange count
 */
function getAIExchangeCount(threadId) {
  return threadAIExchanges.get(threadId) || 0;
}

/**
 * AR-38: Send NPC email using template (convenience function)
 * @param {string} campaignId - Campaign ID
 * @param {string} npcType - NPC type
 * @param {string} templateType - Template type
 * @param {Object} templateData - Data for template
 * @param {Object} mailOptions - Additional mail options (recipientId, recipientType, sentDate)
 * @returns {Object} Created mail
 */
function sendNPCEmail(campaignId, npcType, templateType, templateData, mailOptions = {}) {
  const generated = generateNPCEmail(npcType, templateType, templateData);

  return sendMail(campaignId, {
    senderName: templateData.patronName || templateData.starportName || templateData.companyName || 'NPC',
    senderType: 'npc',
    recipientType: mailOptions.recipientType || 'ship',
    recipientId: mailOptions.recipientId || null,
    subject: generated.subject,
    body: generated.body,
    priority: generated.priority || mailOptions.priority || 'normal',
    sentDate: mailOptions.sentDate || new Date().toISOString()
  });
}
...
module.exports = {
  ...
  // AR-38: NPC Email functions
  NPC_TEMPLATES,
  generateNPCEmail,
  sendNPCEmail,
  checkAIExchangeLimit,
  getAIExchangeCount,
  AI_EXCHANGE_LIMIT
};
