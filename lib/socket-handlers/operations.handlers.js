...
function register(socket, io, state) {
  ...
  socket.on('ops:completeJump', () => {
    try {
      ...
      // Broadcast to ship crew and campaign (include updated time)
      ...
      socketLog.info(`[OPS] Jump completed, arrived at ${result.arrivedAt}, date: ${updatedCampaign.current_date}`);
    } catch (error) {
