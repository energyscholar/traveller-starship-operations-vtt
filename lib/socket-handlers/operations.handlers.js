...
function register(socket, io, state) {
  ...
  socket.on('ops:createCampaign', (data) => {
    try {
      // AR-16.8: Rate limit sensitive operations
  ...
  socket.on('ops:assignRole', (data) => {
    try {
      ...
      // AR-16.6: Validate role against allowed values
  ...
  // Get full template data for editor (AR-17)
  ...
  // Add custom ship (not from template) (AR-17)
  socket.on('ops:addCustomShip', (data) => {
    try {
      ...
      // Add custom ship with fully specified data
      const ship = operations.addShip(opsSession.campaignId, name, shipData, {
