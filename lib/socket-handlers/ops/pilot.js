...
function register(ctx) {
  ...
  socket.on('ops:setEvasive', async (data) => {
    try {
      ...
      broadcastStateChange(io, `ops:campaign:${campaignId}`,'ops:evasiveChanged', {
        enabled,
  ...
  socket.on('ops:setRange', async (data) => {
    try {
      ...
      if (newRange !== currentRange) {
        ...
        broadcastStateChange(io, `ops:campaign:${campaignId}`,'ops:pilotCheck', {
          contactId,
        ...
        broadcastStateChange(io, `ops:campaign:${campaignId}`,'ops:rangeChanged', {
          contactId,
          newRange,
  ...
  socket.on('ops:setCourse', async (data) => {
    try {
      ...
      // Broadcast
      broadcastStateChange(io, `ops:campaign:${campaignId}`,'ops:courseChanged', {
        destination,
        eta,
        travelTime,
        setBy: opsSession.playerName || 'Pilot',
  ...
  socket.on('ops:clearCourse', async () => {
    try {
      ...
      broadcastStateChange(io, `ops:campaign:${campaignId}`,'ops:courseCleared', {
        setBy: opsSession.playerName || 'Pilot',
        timestamp: new Date().toISOString()
      });
  ...
  socket.on('ops:passTime', async (data) => {
    try {
      ...
      broadcastStateChange(io, `ops:campaign:${campaignId}`,'ops:timeUpdated', {
        currentDate: newDate,
  ...
  socket.on('ops:setTimeBlocked', async (data) => {
    try {
      ...
      // Broadcast
      broadcastStateChange(io, `ops:campaign:${campaignId}`,'ops:timeBlockedChanged', {
  ...
  socket.on('ops:travel', async (data) => {
    try {
      const { destinationId, travelHours: clientTravelHours } = data;
      const campaignId = opsSession.campaignId;
      ...
      const currentSystemHex = currentState.systemHex;
      ...
      let system = null;
      if (currentSystemHex) {
        system = starSystems.getSystemByHex(currentSystemHex);
      }
      if (!system && campaign.current_system) {
        system = starSystems.getSystemByName(campaign.current_system);
      }
      if (!system) {
        socket.emit('ops:error', { message: `Unknown star system: ${currentSystemHex || campaign.current_system}` });
        return;
      }

      // Find destination location (use getAllDestinations for dynamic + static)
      const allDestinations = starSystems.getAllDestinations(system.id);
      const destLocation = allDestinations.find(loc => loc.id === destinationId);
      if (!destLocation) {
        socket.emit('ops:error', { message: `Unknown destination: ${destinationId}` });
        return;
      }

      // AR-267: Use client-provided travel hours (calculated from physics), or fall back to preconfigured/default
      let travelHours = clientTravelHours || 4;
      if (!clientTravelHours && currentLocationId && destLocation.travelTimeHours) {
      ...
      broadcastStateChange(io, `ops:campaign:${campaignId}`,'ops:travelComplete', {
        systemHex: currentSystemHex,
  ...
  socket.on('ops:undock', async () => {
    try {
      ...
      // Find current location (use getAllDestinations for dynamic + static)
      const allDestinations = starSystems.getAllDestinations(system.id);
      const currentLocation = allDestinations.find(loc => loc.id === currentLocationId);
      ...
      // Broadcast undock
      broadcastStateChange(io, `ops:campaign:${campaignId}`,'ops:undocked', {
        systemHex: currentSystemHex,
        systemName: system.name,
  ...
  // AR-299: Get destinations for current system (ensures client uses server-generated IDs)
  socket.on('ops:getDestinations', () => {
    try {
      const campaignId = opsSession.campaignId;
      if (!campaignId) {
        socket.emit('ops:destinations', { destinations: [], error: 'No campaign selected' });
        return;
      }

      // Get current system from campaign
      const campaign = operations.getCampaign(campaignId);
      if (!campaign?.current_hex || !campaign?.current_sector) {
        socket.emit('ops:destinations', { destinations: [], error: 'No current system' });
        return;
      }

      // Get system by hex (single parameter)
      const system = starSystems.getSystemByHex(campaign.current_hex);
      if (!system) {
        // Try by name as fallback
        const systemByName = starSystems.getSystemByName(campaign.current_system);
        if (!systemByName) {
          socket.emit('ops:destinations', { destinations: [], error: `System not found: ${campaign.current_hex}` });
          return;
        }
        // Use the system found by name
        const destinations = starSystems.getAllDestinations(systemByName.id);
        socket.emit('ops:destinations', {
          destinations,
          systemId: systemByName.id,
          systemName: systemByName.name,
          hex: campaign.current_hex
        });
        return;
      }

      // Get all destinations using server's authoritative data
      const destinations = starSystems.getAllDestinations(system.id);
      ...
      console.error('[Pilot] Error getting destinations:', error);
      socket.emit('ops:destinations', { destinations: [], error: error.message });
    }
  });
}
