...
function register(ctx) {
  ...
  socket.on('chat:getHistory', () => {
    try {
      ...
      const myName = opsSession.playerName;

      // AR-EPSILON-B: Fixed whisper filtering
      // Match by accountId first, then by playerName as fallback for reconnecting users
      const messages = allMessages.filter(msg => {
        if (msg.type !== 'whisper') return true;

        // Match by ID (accountId or socket.id)
        const idMatch = msg.senderId === myId || msg.recipientId === myId;
        if (idMatch) return true;

        // Fallback: match by playerName for reconnecting users
        if (myName) {
          return msg.senderName === myName || msg.recipientName === myName;
        }
