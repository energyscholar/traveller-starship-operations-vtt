...
const { isAuthEnabled } = require('../../config/auth-config');
...
function register(ctx) {
  ...
  socket.on('ops:selectCampaign', (data) => {
    try {
      ...
      // Check campaign membership if auth is enabled
      if (isAuthEnabled() && socket.user) {
        ...
        if (userRole !== 'admin') {
          ...
          if (!membership) {
            socket.emit('ops:error', { message: 'Not authorized to access this campaign' });
            socketLog.warn(`[OPS] User ${userId} denied access to campaign ${campaignId}`);
            return;
          }
        }
      }
