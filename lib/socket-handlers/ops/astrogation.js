...
function register(ctx) {
  ...
  socket.on('ops:initiateJump', (data) => {
    try {
      if (!opsSession.shipId) {
        socketLog.error('[OPS] initiateJump: No shipId in session', { sessionRole: opsSession.role, sessionCampaign: opsSession.campaignId });
        socket.emit('ops:error', { message: 'Not on a ship (session lost - try refreshing)' });
        return;
      }
      ...
      const { destination, distance = 1, destinationHex, destinationSector } = data;
      if (!destination) {
        socket.emit('ops:error', { message: 'Destination required' });
        return;
      }
      // AR-168: Pass hex/sector UIDs for reliable lookup on jump completion
      const result = operations.initiateJump(opsSession.shipId, opsSession.campaignId, destination, distance, destinationHex, destinationSector);
      if (!result.success) {
  ...
  socket.on('ops:completeJump', () => {
    try {
      ...
      // hex/sector come from result (resolved in completeJump using stored UIDs)
