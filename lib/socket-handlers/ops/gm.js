...
const drills = require('../../operations/drills');
const { validateUUID, validateString } = require('../../operations/validators');
...
function register(ctx) {
  const { socket, io, opsSession, operations, validators, socketLog, sanitizeError } = ctx;
  ...
  socket.on('ops:hailContact', (data) => {
    try {
      ...
      const idResult = validators.contactId(contactId);
      if (!idResult.valid) {
        socket.emit('ops:hailResult', { success: false, message: idResult.error });
        return;
      }
  ...
  socket.on('ops:broadcastMessage', (data) => {
    try {
      const { message } = data;
      const msgResult = validateString(message, { fieldName: 'message', required: true, maxLength: 2000 });
      if (!msgResult.valid) {
        socket.emit('ops:error', { message: msgResult.error });
        return;
      }
  ...
  socket.on('ops:addReveal', (data) => {
    try {
      ...
      const campResult = validators.campaignId(data.campaignId);
      if (!campResult.valid) {
        socket.emit('ops:error', { message: campResult.error });
        return;
      }
      const titleResult = validateString(data.title, { fieldName: 'title', required: true, maxLength: 200 });
      if (!titleResult.valid) {
        socket.emit('ops:error', { message: titleResult.error });
        return;
      }
  ...
  socket.on('ops:updateReveal', (data) => {
    try {
      ...
      const idResult = validateUUID(data.revealId, 'revealId');
      if (!idResult.valid) {
        socket.emit('ops:error', { message: idResult.error });
        return;
      }
  ...
  socket.on('ops:revealNPC', (data) => {
    try {
      ...
      const idResult = validateUUID(data.npcId, 'npcId');
      if (!idResult.valid) {
        socket.emit('ops:error', { message: idResult.error });
        return;
      }
  ...
  socket.on('ops:revealLocation', (data) => {
    try {
      ...
      const idResult = validateUUID(data.locationId, 'locationId');
      if (!idResult.valid) {
        socket.emit('ops:error', { message: idResult.error });
        return;
      }
  ...
  socket.on('ops:triggerEvent', (data) => {
    try {
      ...
      const idResult = validateUUID(data.eventId, 'eventId');
      if (!idResult.valid) {
        socket.emit('ops:error', { message: idResult.error });
        return;
      }
  ...
  socket.on('ops:sendEmail', (data) => {
    try {
      ...
      const idResult = validateUUID(data.emailId, 'emailId');
      if (!idResult.valid) {
        socket.emit('ops:error', { message: idResult.error });
        return;
      }
  ...
  socket.on('ops:shareHandout', (data) => {
    try {
      ...
      const idResult = validateUUID(data.handoutId, 'handoutId');
      if (!idResult.valid) {
        socket.emit('ops:error', { message: idResult.error });
        return;
      }
  ...
  socket.on('ops:toggleModule', (data) => {
    try {
      ...
      const idResult = validateUUID(moduleId, 'moduleId');
      if (!idResult.valid) {
        socket.emit('ops:error', { message: idResult.error });
        return;
      }
  ...
  socket.on('ops:unlockContent', (data) => {
    try {
      ...
      const idResult = validateUUID(contentId, 'contentId');
      if (!idResult.valid) {
        socket.emit('ops:error', { message: idResult.error });
        return;
      }
  ...
  // --- AR-BD-0a: Ship Template Handlers ---

  socket.on('ops:getContactTemplates', () => {
    try {
      const templates = ContactFactory.getTemplates();
      socket.emit('ops:contactTemplates', { templates });
    } catch (error) {
      socket.emit('ops:error', sanitizeError('Templates', error));
      socketLog.error('[OPS] Error getting ship templates:', error);
    }
  });

  socket.on('ops:createContactFromTemplate', (data) => {
    try {
      if (!opsSession.isGM) {
        socket.emit('ops:error', { message: 'Creating contacts requires GM privileges' });
        return;
      }
      const { campaignId, templateId, overrides = {} } = data;
      const campResult = validators.campaignId(campaignId);
      if (!campResult.valid) {
        socket.emit('ops:error', { message: campResult.error });
        return;
      }
      if (!templateId || typeof templateId !== 'string') {
        socket.emit('ops:error', { message: 'Template ID required' });
        return;
      }

      // Create contact data from template
      const contactData = ContactFactory.fromTemplate(templateId, overrides);

      // Add to campaign
      const newContact = operations.addContact(campaignId, contactData);
      io.to(`ops:campaign:${campaignId}`).emit('ops:contactAdded', { contact: newContact });
      socketLog.info(`[OPS] Contact created from template ${templateId} in campaign ${campaignId}`);
    } catch (error) {
      socket.emit('ops:error', sanitizeError('Contact', error));
      socketLog.error('[OPS] Error creating contact from template:', error);
    }
  });

  // --- AR-BD-12: Drill Control Handlers ---

  socket.on('ops:getAvailableDrills', () => {
    try {
      const drillList = drills.getAvailableDrills();
      socket.emit('ops:availableDrills', { drills: drillList });
    } catch (error) {
      socket.emit('ops:error', sanitizeError('Drills', error));
      socketLog.error('[OPS] Error getting available drills:', error);
    }
  });

  socket.on('ops:loadDrill', (data) => {
    try {
      if (!opsSession.isGM) {
        socket.emit('ops:error', { message: 'Loading drills requires GM privileges' });
        return;
      }
      const { campaignId, filename, shipId, shipState } = data;
      const campResult = validators.campaignId(campaignId);
      if (!campResult.valid) {
        socket.emit('ops:error', { message: campResult.error });
        return;
      }
      const fnResult = validateString(filename, { fieldName: 'filename', required: true, maxLength: 200 });
      if (!fnResult.valid) {
        socket.emit('ops:error', { message: fnResult.error });
        return;
      }

      // Load drill with optional ship state snapshot
      const result = drills.loadDrill(campaignId, filename, { shipId, shipState });

      // Broadcast drill loaded to all campaign members
      io.to(`ops:campaign:${campaignId}`).emit('ops:drillLoaded', {
        drill: result.drill,
        contacts: result.contacts,
        cleared: result.cleared,
        startConditions: result.startConditions
      });

      // Also emit contactAdded for each contact (for clients that track contacts separately)
      for (const contact of result.contacts) {
        io.to(`ops:campaign:${campaignId}`).emit('ops:contactAdded', { contact });
      }

      socketLog.info(`[OPS] Drill ${result.drill.name} loaded in campaign ${campaignId}`);
    } catch (error) {
      socket.emit('ops:error', sanitizeError('Drill', error));
      socketLog.error('[OPS] Error loading drill:', error);
    }
  });

  socket.on('ops:resetDrill', (data) => {
    try {
      if (!opsSession.isGM) {
        socket.emit('ops:error', { message: 'Resetting drills requires GM privileges' });
        return;
      }
      const { campaignId } = data;
      const campResult = validators.campaignId(campaignId);
      ...
      const result = drills.resetDrill(campaignId);
      if (!result) {
        socket.emit('ops:error', { message: 'No active drill to reset' });
        return;
      }

      // Broadcast drill reset to all campaign members
      io.to(`ops:campaign:${campaignId}`).emit('ops:drillReset', {
        drillId: result.drillId,
        drillName: result.drillName,
        contacts: result.contacts,
        shipSnapshot: result.shipSnapshot
      });

      // Also emit contactAdded for each restored contact
      for (const contact of result.contacts) {
        io.to(`ops:campaign:${campaignId}`).emit('ops:contactAdded', { contact });
      }

      socketLog.info(`[OPS] Drill ${result.drillName} reset in campaign ${campaignId}`);
    } catch (error) {
      socket.emit('ops:error', sanitizeError('Drill', error));
      socketLog.error('[OPS] Error resetting drill:', error);
    }
  });

  socket.on('ops:getActiveDrill', (data) => {
    try {
      const { campaignId } = data;
      const campResult = validators.campaignId(campaignId);
      if (!campResult.valid) {
      ...
      const activeDrill = drills.getActiveDrill(campaignId);
      const drillState = drills.getDrillState(campaignId);
      socket.emit('ops:activeDrillState', {
        drill: activeDrill,
        state: drillState
      });
    } catch (error) {
