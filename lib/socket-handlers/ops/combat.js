...
function emitNarration(io, campaignId, text) {
  broadcastStateChange(io, `ops:campaign:${campaignId}`,'ops:combatNarration', { text });
}
...
function emitTurnPrompt(io, campaignId, phase) {
  ...
  if (prompt) {
    broadcastStateChange(io, `ops:campaign:${campaignId}`,'ops:turnPrompt', { ...prompt, phase });
  }
}
...
function register(ctx) {
  ...
  socket.on('ops:enterCombat', (data) => {
    try {
      ...
      broadcastStateChange(io, `ops:campaign:${opsSession.campaignId}`,'ops:combatStarted', combatState);
  ...
  socket.on('ops:advancePhase', () => {
    try {
      ...
      broadcastStateChange(io, `ops:campaign:${opsSession.campaignId}`,'ops:phaseChanged', {
        phase: newPhase,
        round: newRound
      });
  ...
  socket.on('ops:exitCombat', (data) => {
    try {
      ...
      broadcastStateChange(io, `ops:campaign:${opsSession.campaignId}`,'ops:combatEnded', { outcome });
  ...
  // TODO: Duplicate handler — ship.js:149 also registers ops:getShipWeapons.
  // This one returns SQL ship_weapons rows; ship.js returns ship_data.weapons (JSON blob).
  ...
  socket.on('ops:fireWeapon', (data) => {
    try {
      ...
      // Check sensor lock for Boon (CRB p161: sensor lock grants Boon to attacks)
      const attackShip = operations.getShip(opsSession.shipId);
      const attackShipState = attackShip?.ship_state || {};
      const sensorLockActive = attackShipState.sensorLock && attackShipState.sensorLockId === targetId;
      ...
      if (result.hit && result.damageType === 'ion' && result.powerDrain > 0) {
        ...
        if (drainResult) {
          ...
          broadcastStateChange(io, `ops:campaign:${opsSession.campaignId}`,'ops:targetPowerDrained', {
            targetId,
      ...
      operations.addCombatLogEntry(opsSession.campaignId, {
        ...
        roll_data: {
          ...
          boon: result.modifiers?.boon || false
        },
      ...
      broadcastStateChange(io, `ops:campaign:${opsSession.campaignId}`,'ops:combatAction', {
      ...
      if (result.targetDestroyed) {
        broadcastStateChange(io, `ops:campaign:${opsSession.campaignId}`,'ops:targetDestroyed', {
          contactId: targetId,
          name: target.name,
      ...
      broadcastStateChange(io, `ops:campaign:${opsSession.campaignId}`,'ops:contactsUpdate', {
  ...
  socket.on('ops:fireAsContact', (data) => {
    try {
      ...
      if (result.hit && result.damageType === 'ion' && result.powerDrain > 0) {
        ...
        // Emit power drained event
        broadcastStateChange(io, `ops:campaign:${opsSession.campaignId}`,'ops:pcShipPowerDrained', {
          shipId: targetShipId,
      ...
      if (result.hit) {
        broadcastStateChange(io, `ops:campaign:${opsSession.campaignId}`,'ops:pcShipDamaged', {
          shipId: targetShipId,
          shipName: targetShip.name,
          attacker: contact.name,
          weapon: weapon.name,
      ...
      if (result.destroyed) {
        broadcastStateChange(io, `ops:campaign:${opsSession.campaignId}`,'ops:shipDestroyed', {
          shipId: targetShipId,
          name: targetShip.name,
  ...
  socket.on('ops:toggleAutoFire', (data) => {
    try {
      ...
      broadcastStateChange(io, `ops:campaign:${opsSession.campaignId}`,'ops:autoFireToggled', {
        contactId,
  ...
  socket.on('ops:pauseEnemyFire', (data) => {
    try {
      ...
      broadcastStateChange(io, `ops:campaign:${opsSession.campaignId}`,'ops:enemyFirePaused', {
  ...
  socket.on('ops:resolveEnemyFire', () => {
    try {
      ...
      // Broadcast results
      broadcastStateChange(io, `ops:campaign:${opsSession.campaignId}`,'ops:enemyFireResult', {
        results,
  ...
  socket.on('ops:launchCraft', (data) => {
    try {
      ...
      broadcastStateChange(io, `ops:campaign:${opsSession.campaignId}`,'ops:craftLaunched', {
        craft: contacts,
        callsigns: toLaunch
      });
  ...
  socket.on('ops:alphaStrike', (data) => {
    try {
      ...
      // Broadcast results
      broadcastStateChange(io, `ops:campaign:${opsSession.campaignId}`,'ops:alphaStrikeResult', result);
  ...
  socket.on('ops:recoverCraft', (data) => {
    try {
      ...
      broadcastStateChange(io, `ops:campaign:${opsSession.campaignId}`,'ops:craftRecovered', {
        contactId,
        callsign
      });
  ...
  socket.on('ops:boardShip', (data) => {
    try {
      ...
      // Broadcast results
      broadcastStateChange(io, `ops:campaign:${opsSession.campaignId}`,'ops:boardingResult', {
        targetId,
        targetName: target.name,
        ...result
      });
