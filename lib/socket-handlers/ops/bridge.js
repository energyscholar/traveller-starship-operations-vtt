 ...
 *          joinBridge, startSession, addLogEntry, advanceTime, alertStatus
 */

const { createRoleViewModel, buildViewModelContext } = require('../../viewmodels');
const { broadcastStateChange, broadcastToOthers } = require('../broadcast-helpers');
...
function register(ctx) {
  ...
  socket.on('ops:assignRole', (data) => {
    try {
      ...
      if (!opsSession.isGuest && !hasUnlimitedSlots) {
        ...
        if (!isAvailable) {
          ...
          if (currentHolder && currentHolder.id !== opsSession.accountId) {
            ...
            broadcastStateChange(io, `ops:campaign:${opsSession.campaignId}`,'ops:crewUpdate', {
      ...
      broadcastToOthers(socket, `ops:campaign:${opsSession.campaignId}`,'ops:crewUpdate', {
        shipId: opsSession.shipId,
  ...
  socket.on('ops:joinBridge', (data = {}) => {
    try {
      ...
      broadcastStateChange(io, `ops:bridge:${shipId}`,'ops:crewOnBridge', {
        accountId: opsSession.accountId,
        role,
  ...
  socket.on('ops:startSession', () => {
    try {
      ...
      broadcastStateChange(io, `ops:campaign:${opsSession.campaignId}`,'ops:sessionStarted', {
        gameDate: campaign.current_date,
  ...
  socket.on('ops:addLogEntry', (data) => {
    try {
      ...
      broadcastStateChange(io, `ops:bridge:${opsSession.shipId}`,'ops:logEntry', { entry });
  ...
  socket.on('ops:advanceTime', async (data) => {
    try {
      ...
      broadcastStateChange(io, `ops:campaign:${opsSession.campaignId}`,'ops:timeAdvanced', {
        newDate,
  ...
  socket.on('ops:setAlertStatus', (data) => {
    try {
      ...
      broadcastStateChange(io, `ops:bridge:${opsSession.shipId}`,'ops:alertStatusChanged', { alertStatus });
  ...
  // AR-125.2: Incoming transmission (ship-to-ship comms)
  socket.on('ops:incomingTransmission', (data) => {
    try {
      if (!opsSession.isGM) {
        socket.emit('ops:error', { message: 'Only GM can trigger incoming transmissions' });
        return;
      }
      const { contactName, message, tone } = data || {};
      if (!contactName || !message) {
        socket.emit('ops:error', { message: 'Transmission requires contactName and message' });
        return;
      }
      ...
      if (opsSession.shipId && opsSession.campaignId) {
        ...
        if (campaign) {
          operations.addLogEntry(opsSession.shipId, opsSession.campaignId, {
            gameDate: campaign.current_date,
            entryType: 'comms',
            message: `[COMMS] ${contactName}: "${message}"`,
            actor: contactName
          });
        }
      }

      // Broadcast to all bridge clients
      broadcastStateChange(io, `ops:bridge:${opsSession.shipId}`, 'ops:transmission', {
        contactName,
        message,
        tone: safeTone,
        timestamp: new Date().toISOString()
      });
      socketLog.info(`[OPS] Incoming transmission from ${contactName} (${safeTone})`);
    } catch (error) {
      socket.emit('ops:error', sanitizeError('Comms', error));
      socketLog.error('[OPS] Error handling transmission:', error);
    }
  });

  // AR-125.3: Get cargo manifest
  socket.on('ops:getCargo', () => {
    try {
      if (!opsSession.shipId) {
        socket.emit('ops:error', { message: 'No ship selected' });
        return;
      ...
      const cargo = ship.current_state?.cargo || { capacity: 0, used: 0, manifest: [] };
      socket.emit('ops:cargoManifest', { cargo });
    } catch (error) {
      socket.emit('ops:error', sanitizeError('Cargo', error));
      socketLog.error('[OPS] Error getting cargo:', error);
    }
  });

  // AR-125.3: Update cargo manifest (GM only)
  socket.on('ops:updateCargo', (data) => {
    try {
      if (!opsSession.isGM) {
        socket.emit('ops:error', { message: 'Only GM can update cargo' });
        return;
      }
      if (!opsSession.shipId) {
        socket.emit('ops:error', { message: 'No ship selected' });
      ...
      if (!Array.isArray(manifest)) {
        socket.emit('ops:error', { message: 'Manifest must be an array' });
        return;
      }
      const validLegalities = ['legal', 'restricted', 'illegal'];
      const sanitized = manifest.map(item => ({
        name: String(item.name || '').slice(0, 100),
        tons: Number(item.tons) || 0,
        value: Number(item.value) || 0,
        legality: validLegalities.includes(item.legality) ? item.legality : 'legal'
      }));
      const used = sanitized.reduce((sum, item) => sum + item.tons, 0);
      const ship = operations.getShip(opsSession.shipId);
      const capacity = ship?.current_state?.cargo?.capacity || 0;
      const cargo = { capacity, used, manifest: sanitized };

      operations.updateShipState(opsSession.shipId, { cargo });

      broadcastStateChange(io, `ops:bridge:${opsSession.shipId}`, 'ops:cargoUpdated', { cargo });
      socketLog.info(`[OPS] Cargo updated: ${sanitized.length} items, ${used}t`);
    } catch (error) {
      socket.emit('ops:error', sanitizeError('Cargo', error));
      socketLog.error('[OPS] Error updating cargo:', error);
    }
  });

  // AR-125.4: Get crew roster
  socket.on('ops:getCrewRoster', () => {
    try {
      if (!opsSession.shipId) {
        socket.emit('ops:error', { message: 'No ship selected' });
        return;
      ...
      const players = operations.getPlayersByShip(opsSession.shipId);
      const roster = {
        players: players.map(p => ({
          name: p.character_name || p.slot_name,
          role: p.role,
          status: 'active'
        })),
        npcs: npcCrew.map(npc => ({
          name: npc.name,
          role: npc.role,
      ...
      socket.emit('ops:crewRoster', { roster });
    } catch (error) {
      socket.emit('ops:error', sanitizeError('Crew', error));
      socketLog.error('[OPS] Error getting crew roster:', error);
    }
  });
}
