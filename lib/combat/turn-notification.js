...
const ROLE_ACTIONS = {
  captain: [
    { id: 'tactics', label: 'Apply Tactics bonus', isDefault: true, phase: 'initiative' },
    { id: 'continue', label: 'Continue engagement', isDefault: true, phase: 'actions' },
    { id: 'focus_fire', label: 'Focus fire on target', isDefault: false },
    { id: 'disengage', label: 'Disengage/retreat', isDefault: false },
    { id: 'aggressive', label: 'Aggressive stance', isDefault: false },
    { id: 'defensive', label: 'Defensive stance', isDefault: false }
  ],
  pilot: [
...
// Phase to role mapping (primary role first)
const PHASE_ROLES = {
  initiative: ['captain', 'sensors'],  // Captain can apply Tactics bonus
  manoeuvre: ['pilot'],
  attack: ['gunner', 'sensors'],
  reaction: ['gunner'],
  actions: ['engineer', 'captain'],
  damage: ['damage_control', 'engineer']
};
...
function getRoleMenuItems(role, state, ship) {
  ...
  let actions = ROLE_ACTIONS[normalizedRole];
  if (!actions) return [];

  // Filter by phase if action has phase restriction
  const currentPhase = state?.phase;
  if (currentPhase) {
    actions = actions.filter(a => !a.phase || a.phase === currentPhase);
  }

  // Captain in initiative phase: show tactics with skill level
  if (normalizedRole === 'captain' && currentPhase === 'initiative' && ship?.captain) {
    const tacticsSkill = ship.captain.skill_tactics_naval || 0;
    actions = actions.map(a => {
      if (a.id === 'tactics') {
        return {
          ...a,
          label: `Apply Tactics (skill ${tacticsSkill})`,
          skill: tacticsSkill
        };
      }
      return a;
