...
// AR-216: Weapons that cannot use called shots
const CALLED_SHOT_EXCLUDED_WEAPONS = ['sandcaster', 'ion_cannon', 'ion'];

/**
 * Check if a weapon type can use called shots
 * @param {string} weaponType - Weapon type string
 * @returns {boolean} True if weapon can use called shots
 */
function canUseCalledShot(weaponType) {
  if (!weaponType) return true; // Default allow for unknown weapons
  const normalized = weaponType.toLowerCase().replace(/_/g, ' ');
  return !CALLED_SHOT_EXCLUDED_WEAPONS.some(excluded =>
...
function selectCalledShotTarget(ctx) {
  // AR-216: Use centralized weapon validation
  if (!canUseCalledShot(ctx.weaponType)) return null;
