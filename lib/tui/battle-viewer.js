...
const { runInteractiveScenario } = require('./scenarios/scenario-runner');
const { QSHIP_AMBUSH_CONFIG } = require('./scenarios/scenario-qship-ambush');
const { FLEET_ASSAULT_CONFIG } = require('./scenarios/scenario-fleet-assault');
...
async function waitForScenarioSelection(session) {
  return new Promise((resolve) => {
    ...
    const onData = (key) => {
      ...
      const scenarios = { '1': 'demo1', '2': 'demo2', '3': 'demo3', '4': 'demo4', '5': 'demo5', '6': 'interactive1', '7': 'interactive2' };
  ...
  while (true) {
    ...
    switch (selection) {
      ...
      case 'scenario':
        ...
        if (newScenario === 'interactive1') {
          await runInteractiveScenario(session, QSHIP_AMBUSH_CONFIG);
        } else if (newScenario === 'interactive2') {
          await runInteractiveScenario(session, FLEET_ASSAULT_CONFIG);
        } else if (newScenario) {
