...
const FLEET_ASSAULT_CONFIG = {
  ...
  introText: [
    'Naval intelligence has located the pirate base. Three corsairs defend it.',
    'Astral Dawn leads the strike — six fighters and an armed pinnace deployed.',
    'Mission: suppress the defenders, then bombard the base.',
    'The pirates have nowhere to run. Neither do you.',
    '',
  ...
  actionHandlers: {
    ...
    demand_surrender: async (state, session) => {
      const fleeing = state.enemyFleet.find(e => e.fleeing && !e.fleeHandled);
      if (fleeing) {
        fleeing.fleeHandled = true;
        await resolveSurrenderNegotiation(session, state, fleeing);
      }
    }
  },
