...
function getActiveWeapons(ship, stance) {
  ...
  const barbettes = (ship.barbettes || []).map(b => ({
    ...b,
    mount: 'barbette',
    weapons: [b.id || b.type || 'barbette']
  }));
  const all = [...turrets, ...barbettes];
  switch (stance.weaponFilter) {
    ...
    case 'standoff':
      return all.filter(t => {
        ...
        return t.mount === 'barbette' || weapons.some(w =>
          w.includes('missile') || w.includes('particle') || w.includes('ion')
        );
      });
    case 'none':
      return [];
