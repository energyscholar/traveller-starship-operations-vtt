...
function formatBattleResult(result) {
  ...
  if (result.calledShotsAttempted > 0) {
    const accuracy = Math.round((result.calledShotsHit / result.calledShotsAttempted) * 100);
    lines.push(`  ${WHITE}Called Shots:${RESET}  ${result.calledShotsHit}/${result.calledShotsAttempted} (${accuracy}%)`);
  }
...
function formatExperimentStats(stats) {
  ...
  if (stats.calledShotAccuracy > 0) {
    lines.push(`  │ Called Shot Acc    │ ${formatStatValue(stats.calledShotAccuracy, '%', false)}│`);
  }
