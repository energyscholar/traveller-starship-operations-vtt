...
/**
 * Run an interactive menu loop for a role station
 * Shows menu, waits for key input, calls handler, loops until handler sets screen to 'role'
 * @param {TUISession} session - TUI session for I/O
 * @param {Function} showFn - Function to render the menu (showGunnerMenu, etc.)
 * @param {Function} handleFn - Function to handle input (handleGunnerInput, etc.)
 * @returns {Promise<void>}
 */
async function runInteractiveMenu(session, showFn, handleFn) {
  let exitMenu = false;
  const setScreen = (screen) => {
    if (screen === 'role') exitMenu = true;
  };
  ...
  return new Promise((resolve) => {
    ...
    const onData = (key) => {
      ...
      // Call the role-specific handler
      handleFn(session, key, setScreen);

      if (exitMenu) {
        cleanup();
        resolve();
      }
      // If handler didn't exit, the menu was refreshed by the handler itself
    };
...
async function runRoleMenu(session) {
  while (true) {
    ...
    switch (selection) {
      case 'captain':
        await runInteractiveMenu(session, showCaptainMenu, handleCaptainInput);
        break;

      case 'pilot':
        await runInteractiveMenu(session, showPilotMenu, handlePilotInput);
        break;

      case 'engineer':
        await runInteractiveMenu(session, showEngineerMenu, handleEngineerInput);
        break;

      case 'gunner':
        await runInteractiveMenu(session, showGunnerMenu, handleGunnerInput);
        break;

      case 'sensors':
        await runInteractiveMenu(session, showSensorsMenu, handleSensorsInput);
        break;

      case 'damage-control':
        await runInteractiveMenu(session, showDamageControlMenu, handleDamageControlInput);
        break;
