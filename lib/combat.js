...
function resolveAttack(attacker, target, options = {}) {
  ...
  if (hit) {
    ...
    const damageMultiple = (weapon && weapon.damageMultiple) || 1;
    const isMissile = weapon && (weapon.type === 'missile' || (weapon.id && weapon.id.includes('missile')));
    const effectiveMult = isMissile ? 1 : damageMultiple;
    const rawDamage = damageRoll.total + effect;
    const damage = Math.max(0, rawDamage - target.armor) * effectiveMult;
...
function resolveSpaceCombatAttack(attacker, target, options = {}) {
  ...
  if (hit) {
    const damageRoll = rollDamageDice(roller, weapon.damage || '2d6');
    const effect = attackTotal - targetNumber;
    const damageMultiple = (weapon && weapon.damageMultiple) || 1;
    const isMissile = weapon && (weapon.type === 'missile' || (weapon.id && weapon.id.includes('missile')));
    const effectiveMult = isMissile ? 1 : damageMultiple;
    const rawDamage = damageRoll.total + effect;
    const damage = Math.max(0, rawDamage - target.armour) * effectiveMult;
...
// ============================================================
// STUB: Docking System (Stage 12.1 - Minimal Implementation)
// ============================================================
// Allows ships to dock when at Adjacent range
// Full boarding/capture mechanics deferred to later
function attemptDocking(ship1Pos, ship2Pos) {
  const distance = hexDistance(ship1Pos, ship2Pos);
  const range = rangeFromDistance(distance);

  if (distance === 0) {
    return {
      success: false,
      message: 'Cannot dock - ships are at the same position',
      docked: false
    };
  }
  ...
  if (range === 'adjacent') {
    return {
      success: true,
      message: 'Ships successfully docked',
      docked: true
    };
  }
  ...
  return {
    success: false,
    message: `Cannot dock - ships must be at Adjacent range (currently ${range})`,
    docked: false,
    requiredRange: 'adjacent',
    currentRange: range
  };
}
...
module.exports = {
  ...
  attemptDocking,  // Stage 12.1: Docking stub
  resolveAttack,
  formatAttackResult,
  getAttackBreakdown,
