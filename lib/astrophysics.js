...
function generateGasGiant(stellarClass, index, seed) {
  ...
  return {
    ...
    orbitAU: orbitAU,
    bearing: Math.round(rng() * 360),  // Random orbital position
...
function generateMoons(parent, seed, sectorHex = null) {
  ...
  for (let i = 0; i < moonCount; i++) {
    ...
    moons.push({
      ...
      bearing: Math.round(rng() * 360),  // Random position around parent
      radiusKm: Math.round(radiusKm),
 ...
 * @param {string} systemName - Name of the system (used for mainworld)
 * @returns {Object[]} Array of celestial objects
 */
function generateSystem(sectorHex, uwp, stellarClass, gasGiantCount = 0, systemName = null) {
  ...
  const mainworld = {
    ...
    type: 'Planet',
    name: systemName || 'Mainworld',  // Use system name for mainworld
    isMainworld: true,
    uwp: uwp,
