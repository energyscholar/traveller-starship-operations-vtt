...
function expandSystem(compact, options = {}) {
  ...
  const full = {
    ...
    // AR-XXX: Explicit base flags for destination generation
    hasNaval: bases.naval,
    hasScout: bases.scout,
...
function getSystem(identifier, options = {}) {
  ...
  // Try converting hyphenated ID to name (e.g., "fenl-s-gren" -> "Fenl's Gren")
  if (identifier.includes('-')) {
    // Convert dashes to spaces and handle common patterns
    let normalizedName = identifier.replace(/-s-/g, "'s ").replace(/-/g, ' ');
    // Title case
    normalizedName = normalizedName.replace(/\b\w/g, c => c.toUpperCase());
    const byNormalizedName = getSystemByName(sectorName, normalizedName, options);
