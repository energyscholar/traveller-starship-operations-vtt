...
/**
 * Calculate jump route between two worlds (AR-121)
 ...
 * @param {number} jump - Ship's jump rating (1-6)
 * @returns {Promise<Object>} Route waypoints
 */
async function getJumpRoute(startSector, startHex, endSector, endHex, jump = 2) {
  ...
  return fetchFromTravellerMap('route', {
    start,
    end,
    jump: Math.min(6, Math.max(1, jump))
  });
...
/**
 * Convert coordinates between systems (AR-121)
 * @param {string} sector - Sector name
 * @param {string} hex - Hex coordinate
 * @returns {Promise<Object>} Coordinate data (sector, subsector, galactic)
 */
async function getCoordinates(sector, hex) {
  return fetchFromTravellerMap('coordinates', { sector, hex });
}

/**
 * List all available sectors (AR-121)
 * @returns {Promise<Object>} List of sectors in the universe
 */
async function listSectors() {
  return fetchFromTravellerMap('universe', {});
}
